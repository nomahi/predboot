pred.lasso2 <- function(model,data,B=1000,alpha=0.05,Cores=detectCores()){
...
　　}
  ...
  R1 <- foreach(b = 1:B, .combine = rbind, .packages=c("MASS","pROC","glmnet"), .options.snow = opts) %dopar% {
   ...
   gm1 <- cv.glmnet(x=X.train[,-1], y=train$Y, family="binomial", alpha=1, foldid=train$ID, grouped=FALSE)
