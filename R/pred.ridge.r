pred.ridge <- function(model,data,B=1000,alpha=0.05,Cores=detectCores()){
...
　　}
  ...
  gm1 <- cv.glmnet(x=X.train[,-1], y=data$Y, family="binomial", alpha=0, foldid=data$ID, grouped=FALSE)
